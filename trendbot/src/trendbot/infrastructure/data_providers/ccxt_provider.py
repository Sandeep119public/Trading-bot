...
class CcxtProvider(DataProvider):
    ...
    def __init__(self, quote_currency: str = "USDT", timeframe: str = "1d", rate_limit: bool = True) -> None:
    ...
    def _create_exchange(market_type: str, rate_limit: bool):
        try:
            ...
            return exchange_class({"enableRateLimit": rate_limit, "options": options})
    ...
    def fetch_daily_close_prices(self, symbol: str, start_date: date, end_date: date | None) -> pd.DataFrame:
        pair = self._normalize_symbol(symbol, self.quote_currency)
        since_ms = int(datetime.combine(start_date, datetime.min.time()).replace(tzinfo=timezone.utc).timestamp() * 1000)
        end_ms = None if end_date is None else int(datetime.combine(end_date, datetime.max.time()).replace(tzinfo=timezone.utc).timestamp() * 1000)
    ...
    def _fetch_paginated(self, pair: str, timeframe: str, since_ms: int, end_ms: int | None) -> list[list]:
        all_candles: list[list] = []
        current_since = since_ms
        while True:
            params = {"endTime": end_ms} if end_ms is not None else {}
            candles = self._fetch_with_retry(pair, timeframe, current_since, BINANCE_MAX_CANDLES, params, 3)
            if not candles:
                break
    ...
    def _fetch_with_retry(self, pair: str, timeframe: str, since_ms: int, limit: int, params: dict, max_retries: int) -> list[list]:
        last_error: Exception | None = None
        for attempt in range(max_retries):
            try:
                return self._exchange.fetch_ohlcv(symbol=pair, timeframe=timeframe, since=since_ms, limit=limit, params=params)
...
class BinanceFuturesProvider(CcxtProvider):
    ...
    def _normalize_symbol(symbol: str, quote_currency: str = "USDT") -> str:
        ...
        if "/" in value:
            ...
            return f"{base}/{quote}:{quote}" if quote in {"USDT", "USDC"} else f"{base}/{quote}:USDT"
        if "-" in value:
            base, quote = value.split("-", 1)
